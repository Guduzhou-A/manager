$(function () {
        ...
        table = $('#content_table').DataTable(
            {
                ...
                "rowCallback": function (row, data, index) {
                    ...
                    var enableHtml;
                    if (data.enable) {
                        enableHtml = "<span class='label1'>首页已展示</span>"
                    } else {
                        enableHtml = "<span class='label'>首页未展示</span>"
                    }
                    $('td:eq(2)', row).html(enableHtml);

                    var editHtml ="";
                    if (data.enable) {
                        // editHtml += "<a href='javascript:void(0);' name='edit-brand-status' data-id='" + data.id + "' style='margin-left: 4%;margin-right: 4%' data-toggle='tooltip' data-placement='left' title='禁用'>" +
                        //     "<i class='glyphicon glyphicon-remove'></i>" +
                        //     "</a>";
                    } else {
                        editHtml += "<a href='javascript:void(0);' name='edit-brand-status' data-id='" + data.id + "' style='margin-left: 4%;margin-right: 4%' data-toggle='tooltip' data-placement='left' title='首页展示'>" +
                    ...
                    $("a[name='edit-brand-status']", row).click(editBrandPageStatus);
                ...
                "sAjaxSource": base + "/home/brand/ajax",
    ...
    function editBrandPageStatus() {
        var id = $(this).attr("data-id");
        $.ajax({
            url: base + '/home/brand/status/' + id,
            type: 'POST',
            async: false,
            cache: false,
            dataType: "json",
